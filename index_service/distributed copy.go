...
// 暴露给调用方的不是index_worker，因为一个worker只存放一部分数据
...
func (sentinel *Sentinel) AddDoc(doc types.Document) (int, error) {
	...
	client := NewIndexServiceClient(conn) //proto
...
func (sentinel *Sentinel) Search(query *types.TermQuery, onFlag uint64, offFlag uint64, orFlags []uint64) []*types.Document {
	...
	wg := sync.WaitGroup{}
	wg.Add(len(endpoints))
	for _, endpoint := range endpoints { //遍历所有endpoints
	...
	go func() { //为什么要放到一个子协程里？因为里面有个无限for循环，只有“//1”执行了该for循环才能退出
		for {
			doc, ok := <-resultCh //管道关闭并且没数据了就会不ok
			if !ok {
				break //2
			}
			docs = append(docs, doc)
		} //不ok就说明全部doc都存入docs中了，进入没有缓存的管道
		receiveFinish <- struct{}{} //3
	}()
	wg.Wait()
	close(resultCh) //1。doc都放入resultCh管道中了，就关闭。关闭只是关闭写入。
	<-receiveFinish //4。管道通了，返回搜索到的文件
	return docs
}
