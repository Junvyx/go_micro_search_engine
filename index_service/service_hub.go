...
const (
	SERVICE_ROOT_PATH = "/search_engine/index" //etcd key的prefix
)

// 服务注册中心
// 每台service worker上都部署一个servicehub？每个go进程之创建一个serivehub，单例模式
type ServiceHub struct {
	client             *etcdv3.Client //用来连接到etcd
	...
	if serviceHub == nil {
		hubOnce.Do(func() {
			if client, err := etcdv3.New(
				...
				util.Log.Fatalf("连接不上etcd服务器: %v", err)
			} else {
...
func (hub *ServiceHub) UnRegist(service string, endpoint string) error {
	...
	if _, err := hub.client.Delete(ctx, key); err != nil {
		...
		util.Log.Printf("注销服务%s对应节点%s", service, endpoint)
		return nil
	}
}
...
func (hub *ServiceHub) GetServiceEndpoints(service string) []string {
	...
	if resp, err := hub.client.Get(ctx, prefix, etcdv3.WithPrefix()); err != nil {
		...
		for _, kv := range resp.Kvs {
			path := strings.Split(string(kv.Key), "/") //这里的key是完整的路径，prefix+service+ip
			// fmt.Println(string(kv.Key), path[len(path)-1])
			endpoints = append(endpoints, path[len(path)-1]) //我们只需要ip部分
		}
